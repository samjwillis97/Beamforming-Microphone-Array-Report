...
\subsubsection{Design Verification} \label{sec:Design Verification Testing}
    ...
    Figure \ref{fig:array verification} displays the results of the delay-and-sum algorithm to both of the test, this algorithm was chosen due to it being much less error prone than MUSIC and has previously been tested and shown to work with a real array. The results show that the array does indeed operate as expected as well as the algorithm. The 47 Degree test was close to exact along the azimuth and was estimated to be a few degrees below in elevation. Innacuracy begins to show at the edge of the array for the 10 Degree test with the azimuth estimation being around 10 Degrees off but the elevation estimation was as expected. 
