...
\subsubsection{MUSIC Algorithm} \label{sec:Intro MUSIC}
    \hl{Subspace Methods for Directions-of-Arrival Estimation + MATLAB} The MUSIC algorithm (MUltiple SIgnal Classification) is a subspace method of beamforming relying upon the eigen-decomposition of the spatial co-variance matrix of the received signal data. The co-variance matrix is decomposed into it's eigenvalues and vectors that are then sorted by eigenvalue and separated into noise and signal.
...
\subsubsection{LabVIEW Implementation} \label{sec:DAS Implementation}
    ...
    \hl{using this method to extract sinals from locations as well}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{MUSIC Algorithm} \label{sec:Software MUSIC}
    \hl{say this section is adapted from direction of arrival estimation and another one} When approximating the direction of arrival of signals using the multiple signal classification algorithm the anticipated data for the $M$ arriving signals are modeled as below in Eqn. \ref{eq:MUSIC signal}. 
    \begin{equation}
    ...
    This implies that the noise eigenvectors are orthogonal to the $M$ steering vectors and is the basis of the MUSIC algorithm. Where orthogonal vectors are those whose dot product equals $0$, if plotted in 2D these would be perpendicular. \hl{ref wolfram mathworld}
    
    Once the noise subspace matrix $\mathbf{Q}_n$ has been determined the MUSIC pseudo spectrum of power can be calculated as a function of $\theta$ and $\phi$ as shown in Eqn.
...
\subsubsection{LabVIEW Implementation} \label{sec:MUSIC LabVIEW}
    \hl{how narrowband works, and how it was adapted to wideband, how the calculation of delays for it worked, reducing computational resources for wideband, add pictures, add heatmap results}
    ...
    When applying the MUSIC algorithm to a wideband signal instead of a single frequency there could be multiple. This increases the computational load required as it could be required to iterate through hundreds of frequencies. To reduce the computational requirements the size of the frequency bins of the Fourier transform for each snapshot are calculated using the following equation where $N_s$ is the number of samples in the full signal.
    ...
    For each frequency bin the full narrowband compute as described above is calculated and the resulting power for each frequency at each point is averaged to calculate the estimated direction of arrival of the $M$ wideband signals. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsubsection{Spatial Signal Generator} \label{sec:Signal Generator}
    \hl{usefuleness of testing sampling rate etc.}
...
\subsection{Geometry Design} \label{sec:Array Geometry}
    ...
    A comparison of various array geometries was performed by Zebb Prime and Con Doolan of The University of Adelaide, this included various spiral array geometries that were evaluated by comparing their minimum side lobe levels and beam widths. A comparison of all the arrays for both near-field and far-field was presented but as the purpose of the array in this report is for far-field signals the near field results were not taken into account. The comparison showed the the Underbrink geometry design was overall the best performing closely followed by the Brüel \& Kjær (B\&K) array \hl{ref}. Hence for the choice of array for the proof of concept design the Underbrink array was chosen as it shows promising resolution and minimal side lobes especially for the number of sensors being used.
...
\subsubsection{Second Array} \label{sec:Second Design DAQ}
    \hl{what was intended vs what was used, picture}
    The second array was designed to be used with two NI cRIO-9036's Real Time Targets with FPGA's and 16 NI-9232 C-series Modules. This configuration would have allowed for up to 48 Channels of synchronised 102.4kHz input signals being processed on the cRIO's themselves. Due to unforeseen circumstances this hardware was made inaccessible and unavailable \hl{changed to what, something about 24 channels}. Hence the microphone array used for testing only consisted of 24 microphones with input speeds of 50kHz with all processing taking place on an attached computer. As the array was designed for the use of 45 microphones spanning over 5 arms it had to be cut down, the inner 20 were microphones were chosen and then 4 of the 5 on the next ring of microphones as shown in Figure \hl[fill this]. 
    
    \hl{add figure}
    
    \hl{add figure of freq response comparison}
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Applying to Machine Defects} \label{sec:Machine Defects}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Lab Testing} \label{sec:Lab Testing}
    \hl{sections for early testing, 24 mic testing etc.}
    
    \hl{comparison of algorithms for everything}
    
    Current testing has only involved myself whistling in-front of the array, but this has given promising results. For the last test I was standing above the microphone array whistling while it ran for one second producing the heat-map in Figure \ref{fig:RealWhistle}. This result shows the estimated source to be right at the top of the array in the centre which is fairly accurate to where I was, also present are two relatively major side-lobes towards the bottom right of the heat-map.
    
    \begin{figure} [H]
        \centering
        \includegraphics[keepaspectratio, width = 0.6\textwidth]{Figures/RealWhistle.png}
        \caption{Result of Whistle test}
        \label{fig:RealWhistle}
    \end{figure}
    
    After performing a Fourier transform on the signal it was found the whistle frequency was around 1675Hz this is information as well as its spatial location was vital to test the result in simulation and 3D frequency response. Knowing the location and the frequency a 3D Frequency response was setup with those parameters producing Figure \ref{fig:RealWhistleFreq}. This resulted in a very similar looking heat-map which would indicate that the algorithm and hardware are both working on the test array. The main-lobe and side-lobes all look nearly identical between the real test and the 3D Frequency Response.
    ...
    Further testing is required before being completely confident with the algorithm and array.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{First Design} \label{sec:First Array Testing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Second Design} \label{sec:Second Array Testing}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Defect Detection} \label{sec:Defect Detection Lab Testing}

Now to verify the ability of the array to differentiate different noise and spatially locate them a simple test was performed in a lab environment. To the right hand side 3m from the array a speaker was playing a loud 2kHz tone and in front of the array to the left 8m away was a turbine balance test rig running at 3000rpm, the test was to see if it would be possible to differentiate the sources.
    
    \begin{figure} [H]
        \centering
        \includegraphics[keepaspectratio, width = 0.6\textwidth]{Figures/Raw Sound Waves.png}
